...
def get_dedu_input_data():
    with open('relu_dedu.json', "r") as f:
...
workdir = "."
repeat_number = pd.read_csv(open(workdir+"/relu_repeat_number.csv"))
ALL_NUMBER = np.sum(repeat_number.values)
NUM_Metrics = 21
NUM_DEDU = len(repeat_number.values)


with open(workdir+"/relu_dedu.json", 'r') as f:
    shape_dict = json.load(f)

CASE_NUM = len(shape_dict["input_size"])
kernel_nums = [1 for i in range(CASE_NUM)]


for file in [workdir+"/relu_nsight_result_wotc.csv"]:
    ...
    with open(file,'r') as f:
        ...
        while cur_case < NUM_DEDU:
            for j in range(len(df_list)):
                curnum = 0.0
                for i in range(cur_line, cur_line+kernel_nums[cur_case]):
                    curnum += float(dfmetric[df_list[j]][i])
                df_dict[df_list[j]].append(curnum)
            cur_line += kernel_nums[cur_case]
            cur_case += 1
        ...
        apr_insizes = get_dedu_input_data()
        max_util = 0
        total_cnt = 0
