...
int main(int argc, char **argv) {
  ...
  std::ifstream i("./data/conv2d_perf"+batchsize_str+"_"+kernelsize_str+"_processed.json");
  ...
  clock_t start, end;
  start = clock();
  int BATCH = op_samples["Batch_sizeN"].size();
  for (int sample_iter = 0; sample_iter < BATCH; sample_iter++)
  {
    int N = op_samples["Batch_sizeN"][sample_iter].asInt();
    int C = op_samples["input_channel"][sample_iter].asInt();//输入通道
    int H = op_samples["image_size_H"][sample_iter].asInt();
    int W = op_samples["image_size_W"][sample_iter].asInt();

    int K = op_samples["output_channel"][sample_iter].asInt();//输出通道
    int R = op_samples["kernel_sizeR"][sample_iter].asInt();//filter长
    int S = op_samples["kernel_sizeS"][sample_iter].asInt();//filter宽
    ...
    int algo_t = op_samples["gemmAlgoMin"][sample_iter].asInt();
    ...
    std::cout << "*** " << K << " " << C << " " << R << " " << S << " " << N << " " << H << " " << W << "***"<< std::endl;
  }
  end = clock();
