...
# nohup /usr/local/cuda-11.1/bin/ncu --metrics $metrics --csv --target-processes all python3 ./profile.py > nsight_interpolate_5999.csv &
nohup /usr/local/cuda-11.2/bin/ncu --metrics $metrics --csv --target-processes all python3 ./profile_interpolate_v.py > nsightV.csv &
# nohup /usr/local/cuda-11.1/bin/ncu -o interpolate --set full --metrics $metrics -f python3 -u ./profile.py > hmc.txt &
