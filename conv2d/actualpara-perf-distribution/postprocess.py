...
def append_kernelinfo(perf_list, tc_list, time_list, kernel_data_dict, metric_info, cnt, count_perf, max_perf, min_perf, aihbm_list, rangecnt):
    ...
    for j in range(len(arg)):
        metric_info[arg[j]].append(kernel_data_dict[arg[j]][cnt][0])
...
def print_metricinfo(dfmetric, kerneljson):
    ...
    for i in range(7):
        print(stringrange[i], ' ', count_perf[i]/total_num, ' ', max_perf[i], ' ',min_perf[i])

with open(jsonfile, 'r') as f:
    shape_dict_row = json.load(f)
    CASR_NUM = len(shape_dict_row["N"])
...
for file in files:
    ...
    with open(file,'r') as f:
        ...
        while cur_case < CASR_NUM:
            kernel_num = shape_out['kernel_nums'][cur_case][0]
...
dfm=df_all
df_all.to_csv('./pd_a100_tc.csv', index=False)
