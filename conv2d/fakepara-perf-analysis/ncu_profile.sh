...
for ((batchsize=16;batchsize<=128;batchsize*=2)); do
   ncu --section "LaunchStats" --section "Occupancy" --metrics $metrics,group:memory__shared_table --csv --target-processes all ./conv2d_nsight_profile $batchsize $kernelsize > ./data/conv2d_nsight${batchsize}_${kernelsize}.csv
   ncu -o conv2d --target-processes all -f ./conv2d_nsight_profile $batchsize $kernelsize > ./data/kernel_numbers${batchsize}_${kernelsize}.dat
