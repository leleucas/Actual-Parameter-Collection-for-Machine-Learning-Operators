...
#    /usr/local/cuda-11.2/bin/ncu --section "LaunchStats" --section "Occupancy" --metrics $metrics,group:memory__shared_table --csv --target-processes all python pooling.py $batchsize > ./data/pooling_nsight${batchsize}.csv
#    # /usr/local/cuda-11.2/bin/ncu -o pooling --target-processes all -f python pooling.py $batchsize > ./data/kernel_numbers${batchsize}.dat
#    # ncu -o conv2d --target-processes all -f ./conv2d_nsight_profile $batchsize 1 > ./data/kernel_numbers${batchsize}_1.txt
# done
mode="nearest"
/usr/local/cuda-11.4/bin/ncu --section "LaunchStats" --section "Occupancy" --metrics $metrics,group:memory__shared_table --csv --target-processes all -f python -u profile_interpolate_v.py > test.csv
